<div>
            ...
            <div class="card">
                <div class="card-body p-b-0">
                    ...
                    <ul class="nav nav-tabs customtab2" role="tablist">
                        <li class="nav-item" wire:click="tabChange('role')"> <a
                                class="nav-link @if($tab == 'role') active @endif" data-bs-toggle="tab" href="#role"
                                role="tab" aria-selected="false"><span class="hidden-sm-up"><i
                                        class="ti-role"></i></span> <span class="hidden-xs-down">Role</span></a> </li>
                        <li class="nav-item" wire:click="tabChange('permission')"> <a
                                class="nav-link @if($tab == 'permission') active @endif" data-bs-toggle="tab"
                                href="#permission" role="tab" aria-selected="false"><span class="hidden-sm-up"><i
                                        class="ti-role"></i></span> <span class="hidden-xs-down">Permission</span></a>
                        </li>
                    </ul>
                    <!-- Tab panes -->
                            ...
                            <div class="p-20">
                                <div class="button-group">
                                    <button type="button" class="btn waves-effect waves-light btn-success"
                                        data-bs-toggle="modal" data-bs-target="#role-modal" wire:click="createRole">Add
                                        new role</button>
                                </div>
                                <table class="table">
    ...
    <!-- role modal -->
    <div wire:ignore.self id="role-modal" class="modal" tabindex="-1" aria-labelledby="role-and-permission"
                ...
                <div class="modal-body">
                    <form wire:submit.prevent="submitRole">
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" placeholder="Enter role here"
                    ...
                    <label class="badge bg-success btn m-1" for="permission_no_{{ $permission->id }}">
                        <input type="checkbox" class="form-check-input" id="permission_no_{{ $permission->id }}"
                            value="{{ $permission->id }}" wire:model="selected_permissions.{{ $permission->id }}"
                            wire:click="checkPermission('{{ $permission->name }}')"> {{ $loop->iteration }}) {{
                        $permission->name }}
                    </label>
                    @endforeach
